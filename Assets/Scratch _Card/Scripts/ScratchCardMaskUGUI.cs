...
namespace ScratchCard
{
    ...
    public class ScratchCardMaskUGUI : ScratchCardMask, IBeginDragHandler, IDragHandler
    {
        ...
        public GameObject mainScratchCardObj;
        public event Action OnScratchCardCleared;
        ...
        protected override void OnRevealProgressChanged()
        {
            base.OnRevealProgressChanged();
            float revealProgress = GetRevealProgress();
            if (revealProgress >= 0.35f)
            {
                ClearScratchCard();
            }
        }
        ...
        {
            DisposeTargetTexture();
            OnScratchCardCleared?.Invoke();
            Destroy(mainScratchCardObj, 3f);
        }
