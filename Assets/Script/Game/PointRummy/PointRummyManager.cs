...
public class PointRummyManager : MonoBehaviour
{
    ...
    public static PointRummyManager Instance;
    ...
    public List<PlayerManager> teenPattiPlayers = new List<PlayerManager>();
    public List<PlayerManager> playerSquList = new List<PlayerManager>();
    public PointRummyPlayer player;

    public PlayerManager player1;
    public PlayerManager player2;
    public PlayerManager player3;
    public PlayerManager player4;
    public PlayerManager player5;
    public PlayerManager player6;
    ...
    public List<CardScript> discardedCardDeck;
    ...
    public CardScript cardFromDiscardPile;// this object will be assigned when a card is drawn from discard pile and to ensure that the same card is not discarded again
    ...
    void Start()
    {
        DataManager.Instance.pointValue = DataManager.Instance.betPrice;
    ...
    {
        //Player should have 13 cards and should be their turn
        //deck should have enough cards so that player can draw from it.
        //Using RNG any random card will be selected and that card will become the child object of the right most group of cards the player has
        //After drawing is completed, an event will be called to inform other players that a card is drawn and then it will be checked for sequence and points will be calculated later.
    ...
    public void DiscardButton()
    {
        ...
        {
            var _removedCard = cardsToGroup[0].GetComponent<CardScript>();
    ...
    public void FinishYesButtonClick()
    {
        ...
        if (player.cards.Count == 14 && cardsToGroup.Count == 1)
        {
            ...
            cardsToGroup[0].transform.DOLocalMove(Vector3.zero, 0.05f);
            cardsToGroup[0].transform.DOScale(new Vector3(0.8f, 0.8f, 0.8f), 0.05f);
            player.cards.Remove(cardsToGroup[0].GetComponent<CardScript>());
            cardsToGroup.RemoveAt(0);
            CardsChecker();
    ...
    private IEnumerator GroupCards()//Selected cards will be grouped together
    {
        ...
        List<CardScript> groupedCards = new List<CardScript>();
        for (int i = 0; i < cardsToGroup.Count; i++)
            groupedCards.Add(cardsToGroup[i].GetComponent<CardScript>());
    ...
    public void CardsChecker()//checks each group of cards for valid sequences/impure sequences/ pairs and calculates points
    {
        ...
        for (int i = 0; i < cardsHolder.childCount; i++)
        {
            ...
            for (int j = 0; j < cardsHolder.GetChild(i).childCount; j++)
            {
                var card = cardsHolder.GetChild(i).GetChild(j).GetComponent<CardScript>().card;
    ...
    public void StartGamePlay()
    {
        ...
        for (int i = 0; i < teenPattiPlayers.Count; i++)
        {
            PlayerManager currentPlayer = teenPattiPlayers[i];
    ...
    public void MenuSubButtonClick(int no)
    {
        ...
        if (no == 1)
        {
            ...
            {   
                    ...
                    {
                        float amountLost = DataManager.Instance.pointValue * 80f;
    ...
    public void BetAnim(PlayerManager player, float amount, int priceIndex)
    {
    ...
    public void ShowTextChange()
    {
        ...
        List<PlayerManager> availablePlayer = playerSquList.Where(t => t.gameObject.activeSelf && !t.isPack).ToList();
    ...
    public void ReturnCardsToDeck()//this is the event when player drops/leaves in between the game 
    {
        ...
        for (int j = 0; j < cardsHolder.childCount; j++)
        {
            ...
            for (int i = 0; i < cardsHolder.GetChild(j).childCount; i++)
            {
                ...
                //}
                var card = cardsHolder.GetChild(j).GetChild(i).GetComponent<CardScript>();
                int _indexNo = -1;
    ...
    public void InsertCardDataForSocket()//add card index in list to be sent in socket when game gets over for cards to display on win screen fro other players
    {
        for (int i = 0; i < cardsHolder.childCount; i++)
        {
            for (int j = 0; j < cardsHolder.GetChild(i).childCount; j++)
            {
                var card = cardsHolder.GetChild(i).GetChild(j).GetComponent<CardScript>();
                for (int k = 0; k < cardShuffles.Count; k++)
                {
    ...
    public void FinalResult()
    {
        ...
        {
            winText.text = "You Lost ₹" + (totalScore * DataManager.Instance.pointValue) + " Better Luck Next Time.";
        ...
        if(resultReceivedCount >= DataManager.Instance.joinPlayerDatas.Count)
        {
            ...
            for (int i = 0; i < teenPattiPlayers.Count; i++)
            {
                if (teenPattiPlayers[i].playerNo == player1.playerNo)
                    continue;
                totalPoints += teenPattiPlayers[i].points;
            }
            if (totalScore == 0)
            {
    ...
    public void PlayerDiscardCard(int index, int playerNo)//updating the discard pile in our game after socket player discards card during their turn
    {
        ...
        var _removedCard = newCard.GetComponent<CardScript>();
    ...
    public void PlayerGetCardsReturnedToDeck(List<int> indexNo)
    {
        ...
        {
            CardScript addedCard = new CardScript();
    ...
    void CheckPackTime(PlayerManager packPlayer)
    {
        print("Enter The Check Player");
        List<PlayerManager> livePlayers = new List<PlayerManager>();
        ...
        if (livePlayers.Count == 1)
        {
            ...
            else
            {
                ...
                //}
                InsertCardDataForSocket();
                //isValidDeclaration = true;//if player makes a wrong call then it will not be registered as invalid declaration if this is set to true from here
                FinishGameCallSocket(totalScore, "Complete");
            }
