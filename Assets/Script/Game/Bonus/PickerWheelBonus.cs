...
namespace EasyUI.PickerWheelUIBonus
{
    ...
    public class PickerWheelBonus : MonoBehaviour
    {
        ...
        public WheelPieceBonus[] wheelPieces;

        // Events
        private UnityAction onSpinStartEvent;
        private UnityAction<WheelPieceBonus> onSpinEndEvent;
        ...
        {
            WheelPieceBonus piece = wheelPieces[index];
            ...
            if (piece.isAmount)
            {
                pieceTrns.GetChild(2).GetComponent<Text>().text = (int.Parse(piece.Amount) * 1).ToString();
            }
            else
            {
            ...
            pieceTrns.GetChild(2).GetComponent<Text>().color = piece.c;
            //Line
            ...
            // Manually adjust piece rotation
            float pieceAngle = index * pieceSpacing;
            pieceTrns.RotateAround(wheelPiecesParent.position, Vector3.back, pieceAngle);

            // Manually adjust line rotation
            Transform lineTrns = Instantiate(linePrefab, linesParent.position, Quaternion.identity, linesParent).transform;
            float lineAngle = index * lineSpacing;
        ...
        public void OnSpinEnd(UnityAction<WheelPieceBonus> action)
        {
        ...
        private void CalculateWeightsAndIndices()
        {
            for (int i = 0; i < wheelPieces.Length; i++)
            {
                WheelPieceBonus piece = wheelPieces[i];
        ...
        /*private void OnDrawGizmos()
        {
            ...
            Vector3 startPoint = wheelPiecesParent.position;

            for (int i = 0; i < wheelPieces.Length; i++)
            {
                float pieceAngle = i * pieceSpacing * Mathf.Deg2Rad;
                float lineAngle = i * lineSpacing * Mathf.Deg2Rad;

                Vector3 pieceDirection = new Vector3(Mathf.Sin(pieceAngle), Mathf.Cos(pieceAngle), 0f);
                Vector3 lineDirection = new Vector3(Mathf.Sin(lineAngle), Mathf.Cos(lineAngle), 0f);

                // Draw piece gizmo
                Gizmos.color = Color.white;
                Vector3 piecePosition = startPoint + pieceDirection * radius;
                Gizmos.DrawSphere(piecePosition, 0.1f);

                // Draw line gizmo
                Gizmos.color = Color.red;
                Vector3 linePosition = startPoint + lineDirection * radius;
                Gizmos.DrawLine(startPoint, linePosition);
            }
        }*/
    }
}