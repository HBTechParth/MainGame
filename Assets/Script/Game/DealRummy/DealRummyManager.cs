...
public class DealRummyManager : MonoBehaviour
{
    ...
    public static DealRummyManager Instance;
    ...
    public List<DealRummyPlayerManager> teenPattiPlayers = new List<DealRummyPlayerManager>();
    public List<DealRummyPlayerManager> playerSquList = new List<DealRummyPlayerManager>();
    public DealRummyPlayer player;

    public DealRummyPlayerManager player1;
    public DealRummyPlayerManager player2;
    public DealRummyPlayerManager player3;
    public DealRummyPlayerManager player4;
    public DealRummyPlayerManager player5;
    public DealRummyPlayerManager player6;
    ...
    public List<DealCardScript> discardedCardDeck;
    ...
    public DealCardScript cardFromDiscardPile;// this object will be assigned when a card is drawn from discard pile and to ensure that the same card is not discarded again
    ...
    public int gameCounter;//best of 2/3
    ...
    {
        DataManager.Instance.DebitAmount(DataManager.Instance.betPrice.ToString(), DataManager.Instance.gameId, "DealRummy_Entry-" + DataManager.Instance.gameId, "game", 11);
    }
    void Start()
    {
        Invoke(nameof(DebitAmount), 0.5f);
        ...
        //int wildJoker = UnityEngine.Random.Range(0, cardShuffles.Count - 5);

        //cardShuffles[wildJoker].isWildJoker = true;

        //for (int i = 0; i < cardShuffles.Count; i++)
        //{
        //    if (cardShuffles[wildJoker].cardNo == cardShuffles[i].cardNo)
        //    {
        ...
        //int rng = UnityEngine.Random.Range(0, cardSuffles.Count);//logic to set up the first card for the discard pile
        ...
        //var card = newCard.GetComponent<CardScript>();
        //card.button.interactable = false;
        //card.card = cardSuffles[rng];
        //card.card.cardNo = cardSuffles[rng].cardNo;
        //card.card.cardSprite = cardSuffles[rng].cardSprite;
        //card.card.isWildJoker = cardSuffles[rng].isWildJoker;
    ...
    public void DrawCardFromDeck()
    {
        if (player.cards.Count == 13 && isMyTurn)
        {
            if (closedDeck.Count > 0)
            {
                ...
                var card = newCard.GetComponent<DealCardScript>();
    ...
    public void DiscardButton()
    {
        ...
        {
            var _removedCard = cardsToGroup[0].GetComponent<DealCardScript>();
    ...
    public void FinishYesButtonClick()
    {
        ...
        if (player.cards.Count == 14 && cardsToGroup.Count == 1)
        {
            ...
            cardsToGroup[0].transform.DOLocalMove(Vector3.zero, 0.05f);
            cardsToGroup[0].transform.DOScale(new Vector3(0.8f, 0.8f, 0.8f), 0.05f);
            player.cards.Remove(cardsToGroup[0].GetComponent<DealCardScript>());
    ...
    private IEnumerator GroupCards()//Selected cards will be grouped together
    {
        ...
        List<DealCardScript> groupedCards = new List<DealCardScript>();
        for (int i = 0; i < cardsToGroup.Count; i++)
            groupedCards.Add(cardsToGroup[i].GetComponent<DealCardScript>());
    ...
    public void CardsChecker()//checks each group of cards for valid sequences/impure sequences/ pairs and calculates points
    {
        ...
        for (int i = 0; i < cardsHolder.childCount; i++)
        {
            ...
            for (int j = 0; j < cardsHolder.GetChild(i).childCount; j++)
            {
                var card = cardsHolder.GetChild(i).GetChild(j).GetComponent<DealCardScript>().card;
        ...
        for (int i = 0; i < cardsHolder.childCount; i++)
        {
            ...
            for (int j = 0; j < cardsHolder.GetChild(i).childCount; j++)
            {
                ...
                var card = cardsHolder.GetChild(i).GetChild(j).GetComponent<DealCardScript>().card;
                groupedCards.Add(card);
                //if (isGameComplete && finalCards.Count < 13)
                //{
                //    for (int k = 0; k < cardShuffles.Count; k++)
                //    {
                //        if (card.cardNo == cardShuffles[k].cardNo && card.color == cardShuffles[k].color)
                //        {
                //            finalCards.Add(k);
                //            break;
                //        }
    ...
    public void StartGamePlay()
    {
        ...
        for (int i = 0; i < teenPattiPlayers.Count; i++)
        {
            DealRummyPlayerManager currentPlayer = teenPattiPlayers[i];
    ...
    public void BetAnim(DealRummyPlayerManager player, float amount, int priceIndex)
    {
    ...
    public void ShowTextChange()
    {
        ...
        List<DealRummyPlayerManager> availablePlayer = playerSquList.Where(t => t.gameObject.activeSelf && !t.isPack).ToList();
    ...
    public void ReturnCardsToDeck()//this is the event when player drops/leaves in between the game 
    {
        ...
        for (int j = 0; j < cardsHolder.childCount; j++)
        {
            ...
            for (int i = 0; i < cardsHolder.GetChild(j).childCount; i++)
            {
                ...
                //}
                var card = cardsHolder.GetChild(j).GetChild(i).GetComponent<DealCardScript>();
                int _indexNo = -1;
                for (int k = 0; k < cardShuffles.Count; k++)
                {
    ...
    public void InsertCardDataForSocket()//add card index in list to be sent in socket when game gets over for cards to display on win screen fro other players
    {
        for (int i = 0; i < cardsHolder.childCount; i++)
        {
            for (int j = 0; j < cardsHolder.GetChild(i).childCount; j++)
            {
                var card = cardsHolder.GetChild(i).GetChild(j).GetComponent<DealCardScript>();
                for (int k = 0; k < cardShuffles.Count; k++)
                {
    ...
    public void FinalResult()
    {
        ...
        if (resultReceivedCount >= DataManager.Instance.joinPlayerDatas.Count)
        {
            ...
            {
                float minScore = 9999;
                int winnerPlayerNo = -1;
                Dictionary<int, float> scoreValue = new Dictionary<int, float>();
                for (int i = 0; i < playerSquList.Count; i++)
                {
                    if (playerSquList[i].playerGamePoints < minScore)
                    {
                        minScore = playerSquList[i].playerGamePoints;
                        winnerPlayerNo = playerSquList[i].playerNo;
                    }
                    scoreValue.Add(playerSquList[i].playerNo, playerSquList[i].playerGamePoints);
                }

                var tempD = scoreValue.OrderBy(x => x.Value).ToDictionary(x => x.Key, x=>x.Value);
                if(tempD.First().Value == tempD.ElementAt(1).Value)
                {
                    if(DataManager.Instance.joinPlayerDatas.Count > 2)
                    {
                        ...
                        {
                            winText.gameObject.SetActive(true);
                            winText.text = " You Lost. Better luck next time.";
                            return;
                        }
                    }
                    else if(DataManager.Instance.joinPlayerDatas.Count == 2)
                    {
                        //tiebreaker
                    }
                }
            ...
            else if(gameCounter == 3)//result for tiebreaker
            {
                float minScore = 9999;
                int winnerPlayerNo = -1;
                for (int i = 0; i < playerSquList.Count; i++)
    ...
    public void PlayerDiscardCard(int index, int playerNo)//updating the discard pile in our game after socket player discards card during their turn
    {
        ...
        var _removedCard = newCard.GetComponent<DealCardScript>();
    ...
    public void PlayerGetCardsReturnedToDeck(List<int> indexNo)
    {
        ...
        {
            DealCardScript addedCard = new DealCardScript();
    ...
    void CheckPackTime(DealRummyPlayerManager packPlayer)
    {
        print("Enter The Check Player");
        List<DealRummyPlayerManager> livePlayers = new List<DealRummyPlayerManager>();
